...
4. 選んだ `choices[]` の `command` と、`choices[].params` に自分で埋めた `required_params` / `param_schema` の値を merge した `params` で、`karakuri.sh command <notification_id> <command> '<params-json>'` を最大1回だけ実行する
5. 次の通知が届くまでリクエストを止める。これはゲーム内の `wait` command を選ぶという意味ではない。通知なしに自発的なリクエストを送らない
